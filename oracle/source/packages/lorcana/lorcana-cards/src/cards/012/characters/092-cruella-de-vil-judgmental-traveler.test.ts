...
describe("Cruella De Vil - Judgmental Traveler", () => {
  it("YOU'RE OUT OF FASHION - banishes chosen damaged character when questing after playing another character", () => {
    const testEngine = LorcanaMultiplayerTestEngine.createWithFixture(
      {
        ...
        play: [cruellaDeVilJudgmentalTraveler],
        inkwell: 1,
        deck: 5,
      },
      {
    ...
    expect(testEngine.asPlayerOne().quest(cruellaDeVilJudgmentalTraveler)).toBeSuccessfulCommand();
    expect(testEngine.asPlayerOne().getBagCount()).toBe(1);
    expect(
      testEngine.asPlayerOne().resolvePendingByCard(cruellaDeVilJudgmentalTraveler, {
