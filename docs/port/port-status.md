...
| 2 | Port deck-validation rules | not started (boundary clarified) | | | | | | Step 1 ported deck **constants** (`MIN_DECK_SIZE`/`MAX_INK_TYPES`/`MAX_COPIES_PER_CARD`), **error/result/stats shapes** (`DeckValidationError`, `DeckValidationResult`, `DeckStats`), and the DAC **format types** (`LorcanaFormat`, `CardFormatData`, `DeckFormatResult`, set/format/kind enums) as data shapes only. Step 2 owns the **algorithms** (`validateDeck`/`validateDeckForFormat`/`getDeckFormats`) and the `LORCANA_FORMATS` data table from `decks/validate-deck.ts`, plus the standard-rules `DeckStats` computation. |
