...
describe("Belle's House - Maurice's Workshop (Enchanted)", () => {
  it("reduces item costs by 1 while you have a character at this location", () => {
    const testEngine = LorcanaMultiplayerTestEngine.createWithFixture({
      hand: [expensiveItem],
      play: [
        bellesHouseMauricesWorkshopEnchanted,
        {
          card: workshopHelper,
          atLocation: bellesHouseMauricesWorkshopEnchanted,
        },
      ],
      inkwell: 2,
    });
  ...
  it("does NOT reduce item costs when no character is at the location", () => {
    const testEngine = LorcanaMultiplayerTestEngine.createWithFixture({
      hand: [cheapItem],
      play: [bellesHouseMauricesWorkshopEnchanted, workshopHelper],
      inkwell: 0,
    });
