...
function initSkill(skillName: string, path: string): string | null {
  ...
  try {
    ...
    writeFileSync(exampleScriptPath, EXAMPLE_SCRIPT.replace(/{skill_name}/g, skillName));
    ...
    writeFileSync(exampleReferencePath, EXAMPLE_REFERENCE.replace(/{skill_title}/g, skillTitle));
