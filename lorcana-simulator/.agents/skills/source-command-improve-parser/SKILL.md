---
name: "source-command-improve-parser"
...
Use this skill when the user asks to run the migrated source command `improve-parser`.
...
Migrated from source command `improve-parser` into a Codex skill. Invoke it as `$source-command-improve-parser` and manually rewrite any slash-command behavior that depended on provider-specific runtime expansion.
